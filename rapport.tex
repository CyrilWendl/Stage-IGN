...
\tikzstyle{data} = [diamond, text centered, draw=black, fill=green!30]
\tikzstyle{process} = [rectangle, text centered, draw=black, fill=orange!30]
\tikzstyle{option} = [rectangle, text centered, draw=black]

\tikzstyle{arrow} = [thick,->,>=stealth]

\begin{tikzpicture}[node distance=4cm, level 1/.style={sibling distance=40mm},edge from parent path={(\tikzparentnode.south) -- +(0,-8pt) -| (\tikzchildnode)}, >=latex]

    \node (SPOT6) [data] {SPOT-6};
    \node (S2) [data,right of=SPOT6] {Sentinel-2};
    \node (S2-RF) [process, below of=S2] {RF};
    \node (SPOT6-DCNN) [process, below of=SPOT6] {DCNN};
    
    % root of the the initial tree, level 1
    \node (Fusion) [root, below right of=SPOT6-DCNN] {Fusion}
    % The first level, as children of the initial tree
        child {node[level 2] (c2) {Positioning the nodes}}
        child {node[level 2] (c5) {test}}
    \foreach \name/\it in {fuzzy/1,bayes/2,margin/3
    }{  child {node[level 2] (c\it) {\name}}};


    \draw [arrow] (S2) -- (S2-RF);
    \draw [arrow] (SPOT6) -- (SPOT6-DCNN);
\end{tikzpicture}
