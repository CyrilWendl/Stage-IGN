...
\end{align}
Those fusion rules will be referred to as Bayesian Sum and Bayesian Product.
...
Fusion was carried out preferring the source with the highest margin (Margin Max) as well as using using the probability weighted by the margin for the Bayesian sum and product (Margin Bayesian Sum Weighted, Margin Bayesian Product Weighted).
...
\begin{enumerate}
    ...
    \end{equation}
    Where $V_i(x,y)=\exp\Bigg(\frac{-\big(I_i(x)-I_i(y)\big)^2}{2\big(\overline{I_i(x)-I_i(y)}\big)^2}\Bigg)$,, $dim$ being the dimension of the image $I_{MS}$, $I_i$ the intensity for pixel $x$ in the multispectral image.
...
A more sophisticated approach was tested taking into account the mean membership probability of the building class from the fusion result for each building instead of assigning the probability 1 within buildings, admitting that the class confusion in boundary regions is higher. However, the approach showed similar results and was not used further.
...
\end{align}
One tries to minimize $E_{total}$, thus to make $\Delta E_{total}$ as small as possible. $\lambda$ is calculated as follows:
\begin{equation}
    \begin{split}
    \Delta E_{total}=&E_{data}^{(R_1\cup R_2)}+\lambda E_{compl}^{(R_1\cup R_2)}\\
    &-E_{data}^{(R_1)}-E_{data}^{(R_2)}-\lambda\big(E_{compl}^{(R_1)}+E_{compl}^{(R_2)}\big)\\
    =&(E_{data}^{(R_1\cup R_2)} - E_{data}^{(R_1)} - E_{data}^{(R_2)})\\
    &+\lambda\big(E_{compl}^{(R_1\cup R_2)}-E_{compl}^{(R_1)}+E_{compl}^{(R_2)}\big)
    \end{split}
\end{equation}
\begin{equation}
...
\begin{table}[H]
	...
	\begin{tabular}{p{1.8cm}llllll}\toprule
		...
		\multirow{3}{*}{\parbox{1.8cm}{Fusion: Margin}} & Margin Bayesian Product & 52.6 & 71.9 & 78.2 & 59.6 & 76.7 \\
		& Margin Max & 50.2 & 69.8 & 77.6 & 58.4 & 76.0 \\
		& Margin Bayesian Sum & 50.8 & 70.3 & 77.8 & 58.8 & 76.3 \\\hline
